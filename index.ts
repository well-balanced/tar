...
async function main() {
  ...
  if (!['ng', 'yg'].includes(code)) return

  const court = getCourts(code)
...
async function book(page: Page, target: Target) {
  ...
  for (let i = 1; i < count; ++i) {
    ...
    if (t.match(`${from}`) || t.match(`${to}`)) {
      await page.click(`li:nth-child(${i + 1}) > a > span.box_inn_btm.color10`)
    }
  }
